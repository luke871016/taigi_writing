...
5. **手機**：畫面下方可在「編輯」與「預覽」之間切換；預覽預設適寬顯示，可改為原寸左右滑動查看。練習項目可用 ↑↓ 調整順序。

6. **安裝到主畫面／桌面**：用支援的瀏覽器開啟後，可安裝成獨立應用程式（PWA）。iPhone／iPad 請用 Safari 的分享選單選擇「加到主畫面」。

## 檔案說明

- `index.html` — 主頁（介面與預覽區）
- `styles.css` — 版型、字體、底線樣式、手機版面
- `app.js` — 行編輯、預覽更新、PDF 產生、PWA 安裝
- `manifest.webmanifest` / `sw.js` — 可安裝到主畫面的設定與離線快取
- `icons/` — 應用程式圖示
